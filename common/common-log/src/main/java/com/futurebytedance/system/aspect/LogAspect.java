package com.futurebytedance.system.aspect;
...
public class LogAspect {
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult) {
        try {
            ...
            // 保存数据库
            operatorLogService.saveSysLog(operLog);
        } catch (Exception exp) {
            // 记录本地异常日志
            log.error("==前置通知异常==");
     ...
     * @throws Exception 异常
     */
    public void getControllerMethodDescription(JoinPoint joinPoint, Log log, SysOperLog operatorLog, Object jsonResult) throws Exception {
        // 设置action动作
        operatorLog.setBusinessType(log.businessType().name());
        // 设置标题
        operatorLog.setTitle(log.title());
        // 设置操作人类别
        operatorLog.setOperatorType(log.operatorType().name());
        // 是否需要保存request，参数和值
        ...
        if (log.isSaveResponseData() && !StringUtils.isEmpty(jsonResult)) {
            operatorLog.setJsonResult(JSON.toJSONString(jsonResult));
        }
    }
